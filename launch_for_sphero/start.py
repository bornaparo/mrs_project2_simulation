...
def distribute_random(): #ja dodao
    x = random.uniform(a=-4.5, b=4.5)
    y = random.uniform(a=-4.5, b=4.5)
    return x, y
...
def create_files(resources, config):
    ...
    with open(new_map_file, 'w') as output, open(template_file, 'r') as temp:
        ...
        for k in range(num_of_robots):
            ...
            elif distribution == "random": #ja dodao
                x, y = distribute_random()
...
def main():
    ...
    yaml_file = rospy.get_param('~param_file', default="launch_params") #ja dodao
    with open(package + f'/launch/{yaml_file}.yaml', 'r') as stream:
